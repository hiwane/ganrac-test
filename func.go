...
func (g *Ganrac) setBuiltinFuncTable() {
	...
	g.builtin_func_table = []func_table{
		...
		{"cadprint", 2, 2, funcCADprint, false, "(CAD)* ", ""},
...
func funcCADprint(g *Ganrac, name string, args []interface{}) (interface{}, error) {
	// cell, cells, proj,
	c, ok := args[0].(*CAD)
	if !ok {
		return nil, fmt.Errorf("%s(1st arg) is expected CAD generated by cadinit()", name)
	}

	s, ok := args[1].(string)
	if !ok {
		return nil, fmt.Errorf("%s(2nd arg) is expected string", name)
	}
